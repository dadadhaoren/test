# -*- coding: utf-8 -*-
"""
将 1223 CSV 流水线产出的 `township_enterprise_counts_by_year_csv_2000_2022.csv`
（年份×乡镇 code×n）并入 `township_county_division_2023.csv`，得到**县级以上区划地名**
（`区县_省级`、`区县_地级`、`区县_县级` 及对应区划码等）以及乡镇名（`Name`、乡镇_*），
口径与 `scripts/enterprise/04_merge_division_to_enterprise_counts.py` 一致。

匹配：**乡镇 code** 与区划表 **code** 相等（字符串规范化后）。

默认输出：`township_enterprise_counts_by_year_csv_2000_2022_with_division.csv`

用法：
  python 04_merge_division_to_enterprise_counts_csv.py
  python 04_merge_division_to_enterprise_counts_csv.py -i counts.csv -o out.csv
"""
...
def main() -> None:
    ...
    default_counts = script_dir / "township_enterprise_counts_by_year_csv_2000_2022.csv"
    default_division = test_dir / "township_county_division_2023.csv"
    default_out = script_dir / "township_enterprise_counts_by_year_csv_2000_2022_with_division.csv"

    ap = argparse.ArgumentParser(
        description="1223 企业乡镇计数表并入区划名（省/市/县/乡镇，与 enterprise/04 一致）",
    )
